...
export function DragAndDropContext({
  ...
  const setForecastHint = useApp((state) => state.setForecastHint)
  const clearForecastHint = useApp((state) => state.clearForecastHint)
  ...
  const normalizePumpId = (id: UniqueIdentifier) => {
    ...
    return idString.startsWith('unscheduled-')
      ? idString.replace('unscheduled-', '')
      : idString
  }
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const action =
      active.data.current?.type === 'CALENDAR_EVENT'
        ...
        : 'Scheduled'
    toast.success(
      `${action} ${pump.model} starting on ${dropDate.toLocaleDateString()}`
    )

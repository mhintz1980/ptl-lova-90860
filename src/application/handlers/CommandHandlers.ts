...
} from '../commands/SchedulePumpCommand';
...
import { pumpStageMoved, pumpScheduled, pumpScheduleCleared } from '../../domain/production/events';
...
/**
 * Handler for SchedulePumpCommand.
 */
export class SchedulePumpHandler {
    constructor(
        private readonly pumpRepository: IPumpRepository,
        private readonly eventBus: EventBus
    ) { }

    async execute(cmd: SchedulePumpCommand): Promise<CommandResult> {
        const pump = await this.pumpRepository.findById(cmd.pumpId);
        if (!pump) {
            return { ok: false, error: `Pump not found: ${cmd.pumpId}` };
        }
...
/**
 * Handler for ClearScheduleCommand.
 */
export class ClearScheduleHandler {
    constructor(
        private readonly pumpRepository: IPumpRepository,
        private readonly eventBus: EventBus
    ) { }

    async execute(cmd: ClearScheduleCommand): Promise<CommandResult> {
        const pump = await this.pumpRepository.findById(cmd.pumpId);
        if (!pump) {
            return { ok: false, error: `Pump not found: ${cmd.pumpId}` };
        }

 ...
 *
 * Canonical sequence: QUEUE → FABRICATION → POWDER_COAT → ASSEMBLY → TESTING → SHIPPING → CLOSED
 * Transitions must follow this exact order with no skipping.
 *
 * Note: Uses POWDER_COAT (underscore) internally. Legacy data uses "POWDER COAT" (space).
 * Migration adapters handle the conversion.
 */
